...
### Фото есть только на твоём компьютере (первый раз показать их на домене)

Файлы из `public/uploads/` в Git не попадают. Чтобы они появились на сайте:

1. **Создай папку на сервере** (по SSH/PuTTY):
   ```bash
   mkdir -p /var/www/for-my-kitten/public/uploads
   ```

2. **Скопируй фото с компьютера на сервер** — любым способом:
   - **WinSCP / FileZilla:** подключись к серверу (SFTP), открой на сервере `/var/www/for-my-kitten/public/uploads` и залей туда все файлы из своей локальной папки `public/uploads`.
   - **PowerShell (если настроен OpenSSH/ключ):** из папки проекта на своём ПК:
     ```powershell
     scp -r public/uploads/* root@31.31.197.5:/var/www/for-my-kitten/public/uploads/
     ```
     (подставь свой IP сервера вместо 31.31.197.5, если другой.)

3. **Проверь, что на сервере есть и список фото.** Если ты используешь `DATA_DIR` и копировал данные в `/var/www/for-my-kitten-data/`, там должен быть актуальный `secret-room.json` (скопируй его из локального `src/data/secret-room.json` на сервер в папку данных один раз). Если не используешь DATA_DIR — тогда `src/data/secret-room.json` на сервере должен содержать те же записи с путями `/uploads/...`; при необходимости скопируй этот файл с ПК на сервер в `src/data/`.

4. Перезапусти приложение: `pm2 restart for-my-kitten`. После этого фото в секретной комнате на домене должны отображаться.

### Фото всё равно не отображаются — что проверить

Чаще всего причина в одном из двух: **на сервере пустой список фото** или **нет самих файлов картинок**.

**1. Откуда приложение берёт список фото**

- Если на сервере задана переменная **DATA_DIR** (например для PM2: `DATA_DIR=/var/www/for-my-kitten-data`), то читается файл **из этой папки**: `DATA_DIR/secret-room.json`. Файл `src/data/secret-room.json` из репозитория при этом **не используется**.  
  → Нужно один раз скопировать с твоего ПК файл `src/data/secret-room.json` (с нужными записями `/uploads/...`) на сервер **в папку DATA_DIR**, т.е. в `/var/www/for-my-kitten-data/secret-room.json` (или куда у тебя указывает DATA_DIR).
- Если **DATA_DIR не задан**, список берётся из `src/data/secret-room.json` в проекте (после `git pull` он должен быть из репо). Если там пусто — скопируй свой локальный `src/data/secret-room.json` на сервер в `src/data/` и перезапусти приложение.

**2. Есть ли на сервере сами файлы фото**

- В WinSCP открой на сервере папку **`/var/www/for-my-kitten/public/uploads`** (или папку, на которую указывает симлинк `public/uploads`, если ты настраивал DATA_DIR и перенос загрузок).
- Проверь, что там лежат **те же файлы**, что и в `secret-room.json`: например `mlmde08f-4iic2i.jpg`, `mlmde2qw-xzyg35.jpg` и т.д. Имена должны **совпадать** с путями в JSON (без `/uploads/`, только имя файла).

**3. После правок**

- Выполни на сервере: `pm2 restart for-my-kitten`.
- Открой секретную комнату на сайте, введи пароль и обнови страницу (Ctrl+F5).
